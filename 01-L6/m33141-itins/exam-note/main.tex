...
\begin{document}
\textbf{ITINS Exam Note - 2108121 - May 2026}
...
\textit{IDS:} out-of-band; passive inspection only; triggers logs / alerts after detection. Invented first.\\
\textit{IPS:} physically in-line with core traffic flow path; active mitigation; drops packets; can introduce slight delay for pkt. inspection.
...
Detection rules = signatures, like AV signatures. \textit{Atomic} 1 packet; \textit{composite} many packets. High accuracy for known exploits, blind to zero-days exploits. \textit{Anomaly Based:} uses baseline of normal network behaviour patterns over time, flags metrics outside this, can identify zero-days, notorious for high false-positive rates\\
\textit{Cisco IPS Actions:} Drop, alert, reset TCP connection, isolate

\textsc{Authentication, Authorisation and Accounting}\\
\textit{Methods:} What you know, are, have. PWs, PINs, Biom. Alts: Graphical, Elec. Tattoos, Chip filled pills, SecurID. \\
...
\textit{Adv.:} improves packet fwd. performance,; supports QoS \& CoS; scalable; intergrates IP \& ATM; interoperable. \textit{Disadv.:} additional complexity for router \& additional layer in packet.\\
\textit{MPLS VPN:} Customer Edge Router, Provider Edge Router, Provider Router, AS Border Router
...
\textit{Challenges:} interference (unlicensed spectrum, blame baby monitors, mobile phones, microwaves); health issues. \\ 
...
\textit{Hierarchy:} 802.11i (WPA2 Full std.) \textgreater WPA2 \textgreater WPA \textgreater WEP.\\
\textit{WEP:} Static key, short 24-bit Initialisation Vector with RC4 cipher. Weak IVs repeat quickly, allows packet capture and decrypt root key.\\
\textit{WPA:} Stopgap for WEP. Same hardware as WEP but needs firmware update. Replaces RC4 with TKIP and adds per-packet key hashing.\\
\textit{WPA2:} Robust. Advanced CCMP (counter mode cipher block chaining message authentication code protocol), uses AES. \\
\textit{WPA vs. 802.11i:} Common: OPeration within BSS, cipher \& authentication negotiation during initial association, dynamic keys (temp. session keys), key hierarchy (pairwise master key produces pairwise transient key). Different: WPA no pre-authentication (802.11i has it which means client complete 802.1X auth. to new ap before disconnecting from current)
