...
There is, however, a more dangerous side to wireless connected devices; in that the more devices we connect to the Wi-Fi, the more attack vectors there are on our internal network and clues into our lives. Take, for example, a smart, autonomous vacuum cleaner who can pootle around and suck up dust. This device may have a camera as part of it's mapping and collision detection equipment. This camera may inadvertently capture something such as where valuables are kept which is highly valuable information for burglars. Alternatively, they may map the property and transmit this over the internet, showing the burglars a floor plan of the property - enabling them to plan a more sophisticated attack. 
...
WLAN devices can pose health issues. High power radio waves are not good for the human body, as they can do bad things to cells. Wireless access points contain high power radios and therefore can do bad things to the body. 
...
\begin{itemize}
    ...
    \item Technology weakness - this is some issue with the technology, such as signals extending too far, using a shared spectrum or client isolation being incorrectly configured.
...
\item[Authentication Server] EAP Plug-in within radius server  
\end{define}

\subsection{Wi-Fi Protected Access}
The WLAN industry recognises the vulnerabilities of the so-far existing authentication and data privacy. WEP is not safe! The Wi-Fi Alliance took it upon themselves to make it better, and from here \textit{Wi-Fi Protected Access} (WPA) was born. WPA can also be known as Simple Secure Networking (SSN).
...
\begin{extlink}
There are diagrams in the slides available on Moodle which show the way the per-frame keying works on a technical level. This is ``too mathematical'' so will not be included in the Exam.
\end{extlink}
...
The \textit{Wireless Intrusion Detection System} (WIDS) / \textit{Wireless Intrusion Prevention System} (WIPS) is wireless IPS / IDS. It is built into WLCs. WIDS enables monitoring of rogue APs. WIPS allows proactive action to deny services which can lead to the rogue devices being `contained.'