...
We can use a recursion tree to solve the time complexity of a merge sort. The recursion tree can show how we decompose a statement, for example $C \times n$ into two smaller sub problems. 

If we take the above recurrence to solve $T(n) = 2T(n/2) + C \times n$, where $C>0$ is a constant. We can see how this is first split into two equations of $T(n/2)$
...
\node{$C \times n$}   
    ...
    child {node {$T(n / 2)$}};
\end{tikzpicture}
\caption{First level of recursion tree}
\end{figure}

After this, the branches subdivide again so they are now $T(n/4)$.

\begin{figure}[H]
\centering
\begin{tikzpicture}[level 1/.style={sibling distance=4cm}, level 2/.style={sibling distance=2cm}]
\node{$C \times n$}   
    child {node {$C \times n / 2$}
        child {node {$T(n/4)$}}
        child {node {$T(N/4)$}}}
    child {node {$C \times n / 2$}
        child {node {$T(n/4)$}}
        child {node {$T(N/4)$}}};
\end{tikzpicture}
\caption{Second level of recursion tree}
\end{figure}

This subdivision will continue down the layers of branches until we reach the base case, where the leaves are $Const \times n$. The total number of leaves in the tree will equal $n$.
