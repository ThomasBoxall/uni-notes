...
\subsection{Self Reference (Paradox)}
...
\subsection{Diagonalisation - Countable Sets}
...
However if the sets are infinite - i.e. a set of all the natural numbers $\mathbb{N}$, then we have no easy way of telling how big the set is so therefore we can't pair off in the same way. 
...
Diagonalisation is the idea that there are some sets $P(S)$ in which some elements (which are sets) contain different elements from another subset of $P(S)$, which therefore shows that your original set does not contain all the possibilities of the infinite elements which therefore proves that the original set is not countable. This can be seen particularly with power sets. 
