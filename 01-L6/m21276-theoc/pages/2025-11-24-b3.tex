...
There are a couple of concepts which we need to be familiar with to be able to understand this lecture. We will see this in the form of an example.
...
\subsection{Tool 2: Diagonalization}
...
We can use $\mathbb{N}$ to our advantage when referring to infinite sets; as we known $\mathbb{N}$ itself is infinite. Therefore we can define a set as \textit{countably infinite} if it is the same size as $\mathbb{N}$. 
...
\begin{example}{Diagonalisaiton 1}
...
We have proven, by contradiction, that such a list does not exist and therefore $P(\mathbb{N})$ is not countable. 
