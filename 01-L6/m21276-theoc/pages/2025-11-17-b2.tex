...
If we consider the function $exp(x,y)$ which finds the $y$ exponent of $x$ (meaning $x^y$), we know that
\begin{align*}
    x^0 &= 1\\
    x^1 &= x\\
    x^2 &= x \cdot x\\
...
From this exercise of inductive decomposition, we've found that primitive recursion is in the spirit of ``computation by rewriting'' for definitions of $exp$, $\cdot$ and $+$. If we want to use ``recursion'', it consists of one rule for $y=0$ and one rule for $y>0$ where $y$ acts as a countdown for the number of remaining steps in the computation.
