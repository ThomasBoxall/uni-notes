...
add something about the pcf vs dcf and what they mean: ie dcf means ap is in sleep mode so all distributed vs when in pcf the ap is controlling the comms (contention-free mode).
\end{todo}
...
rts (request to send), cts (clear to send) - at least acknowledge the acronym.
\end{todo}
...
DCF makes use of a set of delays that amount to a priority scheme. 
