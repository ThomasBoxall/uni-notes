...
Asymmetric Encryption is an encryption method where the sender and receiver both use different keys to encode and decode the message. This is where it's alternative name, \textit{Public Key Encryption} comes from.
...
\node[actor] (sender) at(-5,0) {A};
\node[actor] (receiver) at(5,0) {B};
\node[actor] (adversary) at(0,-3) {T};

\draw[->, thick] 
    (sender.south) 
    -- ++(0, -1) coordinate (path_start)
    -- (path_start -| receiver.south) coordinate (path_end)
    -- (receiver.south);

\draw[->] (adversary.north) ++ (-0.2, 0) -- (-0.2, -1.5);
\draw[->] (0.2, -1.5) -- ($(adversary.north) + (0.2, 0)$);

\node at ($(sender.west) + (-0.75, 0)$) {$K_{AB}$};
\node at ($(receiver.east) + (0.75, 0)$) {$K_{AB}$};

\node at ($(sender.south east) + (0.8, -0.75)$) {$C=E_{K_{AB}}(M)$};
\node at ($(receiver.south west) + (-0.8, -0.75)$) {$M=D_{K_{AB}}(C)$};

\end{tikzpicture}
\caption{Example of Symmetric Key Encryption with a sender, A, receiver, B, and adversary, T showing confidentiality}
\end{figure}
...
\node at ($(sender.west) + (-0.75, 0)$) {$K_{A+}$};
\node at ($(sender.west) + (-0.75, 0.5)$) {$K_{A-}$};
\node at ($(sender.west) + (-0.75, -0.5)$) {$K_{B+}$};

\node at ($(receiver.east) + (0.75, 0)$) {$K_{B+}$};
\node at ($(receiver.east) + (0.75, 0.5)$) {$K_{B-}$};
\node at ($(receiver.east) + (0.75, -0.5)$) {$K_{A+}$};

\node at ($(adversary.west) + (-0.75, 0)$) {$K_{A+}$};
\node at ($(adversary.west) + (-0.75, 0.5)$) {$K_{B+}$};
\node at ($(adversary.west) + (-0.75, -0.5)$) {$K_{T+}$};
\node at ($(adversary.west) + (-0.75, -1)$) {$K_{T-}$};


\node at ($(sender.south east) + (0.8, -0.75)$) {$C=E_{K_{B+}}(M)$};
\node at ($(receiver.south west) + (-0.8, -0.75)$) {$M=D_{K_{B-}}(C)$};

\end{tikzpicture}
\caption{Example of Asymmetric Key Encryption with a sender, A, receiver, B, and adversary, T showing confidentiality}
\end{figure}

As we can see in the above figure, the adversary, T can read whatever is on the transmission network however as it's been encoded using $K_{B+}$ which requires the use of $K_{B-}$ to decode it - they cannot decode it. T can, however, block the message from A ($C$) from being transmitted and transmit their own message in it's place which is encrypted using $K_{B+}$ which makes it look like A sent it, because T has B's public key in their keyring. 
...
\node at ($(sender.south east) + (0.8, -0.75)$) {$C=E_{K_{A-}}(M)$};
\node at ($(receiver.south west) + (-0.8, -0.75)$) {$M=D_{K_{A+}}(C)$};

\end{tikzpicture}
\caption{Example of Asymmetric Key Encryption with a sender, A, receiver, B, and adversary, T showing Integrity through use of Digital Signatures}
\end{figure}
...
\node[actor] (m1) at (-6,2) {M};
\node[actor] (m2) at (-3,0) {M};
\node[actor] (h2) at (-3,-1) {$H(M)$};
\node[actor] (m3) at (2,0) {M};
\node[actor] (h3) at (2,-1) {$H(M)$};

\node[proc] (p1) at (5, 2) {H};
\node[proc] (p2) at (5, -3) {\resizebox{\textwidth}{!}{$D_{K_{A+}}$}};

\node[proc] (p3) at (-6, -1) {H};
\node[proc] (p4) at (-6, -3) {\resizebox{\textwidth}{!}{$E_{K_{A-}}$}};
...
\draw[->] (m2.south east) -- (m3.south west) node[midway, above] {$(M, K_{A-}(H(M)))$};

\draw[->] (m1.south) -- (p3.north);
\draw[->] (p3.south) -- (p4.north) node[midway, left] {$H(M)$};
\draw[->] (p4.east) -- (h2.west) node[midway, right] {$E_{K_{A-}}(H(M))$};

\draw[->] (m3.east) -- (p1.west);
\draw[->] (h3.east) -- (p2.west);

\draw[->] (p1.east) -- (o1.west);
\draw[->] (p2.east) -- (o2.west);


\end{tikzpicture}
\caption{Example of Message Digests transmission}
\end{figure}
