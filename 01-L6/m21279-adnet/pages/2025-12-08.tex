...
The alternative to this is a logically centralised distributed control plane. This comprises of physically distributed elements that interface to each other through east \& west bound interfaces. The dta plane and the applications only see a single logical controller.
...
In reactive mode, the controller is queried when packets arrive and there is no entry or an expired timer has expired for the required destination. The controller creates and installs a rule in the corresponding packet if necessary. Importantly, the rule is only installed on switches which will be implicated in the forwarding of this flow. 
